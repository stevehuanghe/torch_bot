...
def get_word_vectors(names, wv_type='glove.6B', wv_dir='./data/glove', wv_dim=300, random=True,
                     multi_avg=True, custom_map=None):
    wv_dict, wv_arr, wv_size = _load_word_vectors(wv_dir, wv_type, wv_dim)

    if custom_map is not None:
        for k in custom_map:
            wv_dict[k.lower()] = wv_dict[custom_map[k]]
...
def _load_word_vectors(root, wv_type, dim):
