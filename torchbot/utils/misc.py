...
def print_param(model, bias=False, names=True):
    """
    ...
    for p_name, p in model.named_parameters():
        ...
        if not bias and not ('bias' in p_name.split('.')[-1] or 'bn' in p_name.split('.')[-1]):
            st[p_name] = ([str(x) for x in p.size()], np.prod(p.size()), p.requires_grad)
        elif bias and not 'bn' in p_name.split('.')[-1]:
            st[p_name] = ([str(x) for x in p.size()], np.prod(p.size()), p.requires_grad)
...
def manual_clip_grad_norm(named_parameters, max_norm, clip=False, verbose=False):
    ...
    if verbose:
        ...
        for name, norm in sorted(param_to_norm.items(), key=lambda x: -x[1]):
            print("{:<50s}: {:.6f}, ({})".format(name, norm, param_to_shape[name]))
        print('-------------------------------', flush=True)
...
def sample_negative_labels(gt_labels, num_neg):
    """
    sample some negative classes for ranking loss
    :param gt_labels: [batch, num_class], torch tensor
    :param num_neg: number of negative classes
    :return: neg_labels: [batch, num_class]
    """
    neg_labels = torch.zeros_like(gt_labels)
    inv_labels = 1 - gt_labels.float()
    weight = torch.sum(inv_labels, 1)
    probs = inv_labels / weight.unsqueeze(1)
    num_neg = min(weight.min(), num_neg)
    indices = torch.multinomial(probs, num_neg, replacement=False)
    neg_labels = neg_labels.scatter(1, indices, 1)
    return neg_labels
